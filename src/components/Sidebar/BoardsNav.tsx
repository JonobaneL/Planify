...
import { LuChartColumn, LuPanelLeft, LuPlus, LuSquareKanban, LuTable2 } from "react-icons/lu";
...
const BoardsNav = () => {
  ...
  return (
    <nav className={`${isOpen ? "pt-5" : "pt-3"} border-t`}>
      {path.includes("board") ? (
        <>
          <div className={`${isOpen ? "px-5" : "gap-0 px-3"} mb-3 flex items-center gap-2`}>
            <div className="flex size-10 items-center justify-center rounded bg-primary-b-10 font-semibold text-primary-b shadow-sm">
              B1
            </div>
            {isOpen && <p>Board 1</p>}
          </div>
          <div className={`flex flex-col items-center gap-0.5 ${isOpen ? "px-5" : "px-3"}`}>
        ...
        <>
          ...
          <div className={`flex flex-col gap-0.5 ${isOpen ? "px-6" : "px-3"}`}>
            {boardList.map((route) => (
              <NavItem key={route.id} icon={<LuPanelLeft />} title={route.name} link={route.href} />
            ))}
          </div>
        </>
      )}
